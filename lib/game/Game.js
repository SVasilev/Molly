game = new Phaser.Game(640, 360, Phaser.CANVAS, 'phaser-example', { preload: preload, create: create, update: update, render: render });
...
function create() {
    ...
    //game.world.setBounds(0, 0, 6144, 2300);
...
function collidewithSpikes(player, ivojson) {
    if(ivojson.sprite && player.y + player.sprite.height - ivojson.sprite.height + 1 < ivojson.y) {
...
/*
function fallingUnder(player) {
    if (player.sprite.body.y > 2600.0) {
        if (fallingAudio.isPlaying) {
            return;
        } else {
            fallingAudio.play();
            fallingAudio.fadeOut(6000);
            fallingAudio.stop();
        }
    }
}*/
...
function update() {
    ...
    if (game.input.keyboard.isDown(Phaser.Keyboard.SPACEBAR)) {
        ...
        if(game.player.sprite.body.x + 100 > element.x && game.player.sprite.body.x - 100 < element.x) {
            element.destroy();
            game.player = new Torch(game, 450, JSONlevels[0].height * 32 - 100, 'kurt');
            lights.add(player);
            //Add animations...
            game.player = new Player(player);
        }
