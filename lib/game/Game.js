game = new Phaser.Game(800, 640, Phaser.CANVAS, 'phaser-example', { preload: preload, create: create, update: update, render: render });
...
function create() {
    ...
    player = new Torch(game, 450, JSONlevels[0].height * 32 - 100, 'mario');
    lights.add(player);
    ...
    */
    ...
    game.objects["vplatform"].forEach(function(element) {
...
function collidewithEnemies(player, platform) {
	if(player.y + player.sprite.height - platform.sprite.height + 1 < platform.y && player.x > platform.x - platform.sprite.width / 2 && player.x < platform.x + platform.sprite.width - platform.sprite.width / 33) {
		//Kill enemy.
	}
}
